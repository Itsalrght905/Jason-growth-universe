...
export default function JokeCorner({state,age,onView,onFavorite,onCreate}){
  ...
  const daily=state.dailyJokeDate===today&&getJokeById(state.dailyJokeId)?getJokeById(state.dailyJokeId):ageJokes[dayNumber(today)%ageJokes.length]
  ...
  function another(){const index=ageJokes.findIndex(joke=>joke.id===current.id);choose(ageJokes[(index+7)%ageJokes.length])}
